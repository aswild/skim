...
use clap::{crate_version, Command, Arg, ArgAction, ArgMatches};
use skim::prelude::*;
...
fn real_main() -> Result<i32, std::io::Error> {
    ...
    let opts = Command::new("sk")
        ...
        .args_override_self(true) // deprecated in 3.x, but not-deprecated in 4.x, leave it in
        ...
        .arg(Arg::new("prompt").long("prompt").short('p').takes_value(true).default_value("> "))
        .arg(Arg::new("cmd-prompt").long("cmd-prompt").takes_value(true).default_value("c> "))
        .arg(Arg::new("expect").long("expect").action(ArgAction::Append).value_name("KEYS"))
        .arg(Arg::new("tac").long("tac").action(ArgAction::SetTrue))
        .arg(Arg::new("tiebreak").long("tiebreak").short('t').takes_value(true))
        .arg(Arg::new("ansi").long("ansi").action(ArgAction::SetTrue))
        .arg(Arg::new("exact").long("exact").short('e').action(ArgAction::SetTrue))
        .arg(Arg::new("cmd").long("cmd").short('c').takes_value(true))
        .arg(Arg::new("interactive").long("interactive").short('i').action(ArgAction::SetTrue))
        .arg(Arg::new("query").long("query").short('q').takes_value(true))
        .arg(Arg::new("cmd-query").long("cmd-query").takes_value(true))
        .arg(Arg::new("regex").long("regex").action(ArgAction::SetTrue))
        .arg(Arg::new("delimiter").long("delimiter").short('d').takes_value(true))
        .arg(Arg::new("nth").long("nth").short('n').takes_value(true))
        .arg(Arg::new("with-nth").long("with-nth").takes_value(true))
        .arg(Arg::new("replstr").short('I').takes_value(true))
        .arg(Arg::new("color").long("color").takes_value(true))
        .arg(Arg::new("margin").long("margin").takes_value(true).default_value("0,0,0,0"))
        .arg(Arg::new("min-height").long("min-height").takes_value(true).default_value("10"))
        .arg(Arg::new("height").long("height").takes_value(true).default_value("100%"))
        ...
        .arg(Arg::new("preview").long("preview").takes_value(true))
        .arg(Arg::new("preview-window").long("preview-window").takes_value(true).default_value("right:50%"))
        .arg(Arg::new("reverse").long("reverse").action(ArgAction::SetTrue))
        .arg(Arg::new("algorithm").long("algo").takes_value(true).default_value("skim_v2"))
        .arg(Arg::new("case").long("case").takes_value(true).default_value("smart"))
        ...
        .arg(Arg::new("hscroll-off").long("hscroll-off").takes_value(true).default_value("10"))
        .arg(Arg::new("filepath-word").long("filepath-word").action(ArgAction::SetTrue))
        .arg(Arg::new("jump-labels").long("jump-labels").takes_value(true).default_value("abcdefghijklmnopqrstuvwxyz"))
        .arg(Arg::new("border").long("border").action(ArgAction::SetTrue))
        .arg(Arg::new("inline-info").long("inline-info").action(ArgAction::SetTrue))
        .arg(Arg::new("header").long("header").takes_value(true).default_value(""))
        .arg(Arg::new("header-lines").long("header-lines").takes_value(true).value_parser(clap::value_parser!(usize)).default_value("0"))
        .arg(Arg::new("tabstop").long("tabstop").takes_value(true).default_value("8"))
        .arg(Arg::new("no-bold").long("no-bold").action(ArgAction::SetTrue))
        .arg(Arg::new("history").long("history").takes_value(true))
        .arg(Arg::new("cmd-history").long("cmd-history").takes_value(true))
        .arg(Arg::new("history-size").long("history-size").takes_value(true).default_value("1000"))
        .arg(Arg::new("cmd-history-size").long("cmd-history-size").takes_value(true).default_value("1000"))
        ...
        .arg(Arg::new("filter").long("filter").short('f').takes_value(true))
        .arg(Arg::new("layout").long("layout").takes_value(true).default_value("default"))
        .arg(Arg::new("keep-right").long("keep-right").action(ArgAction::SetTrue))
        .arg(Arg::new("skip-to-pattern").long("skip-to-pattern").takes_value(true).default_value(""))
        .arg(Arg::new("pre-select-n").long("pre-select-n").takes_value(true).value_parser(clap::value_parser!(usize)).default_value("0"))
        .arg(Arg::new("pre-select-pat").long("pre-select-pat").takes_value(true).default_value(""))
        .arg(Arg::new("pre-select-items").long("pre-select-items").takes_value(true))
        .arg(Arg::new("pre-select-file").long("pre-select-file").takes_value(true).default_value(""))
    ...
    // WILD NOTE: this used to be a multiple option where args were concatenated with ':'
...
fn parse_options(options: &ArgMatches) -> SkimOptions<'_> {
    SkimOptionsBuilder::default()
        ...
        .min_height(options.get_one("min-height").map(String::as_str))
        .no_height(options.get_flag("no-height"))
        .height(options.get_one("height").map(String::as_str))
